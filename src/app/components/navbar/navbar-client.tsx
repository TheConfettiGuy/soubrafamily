...
// NOTE: no JSON import here—data is passed via props
...
export default function NavbarClient({
  ...
  data: {
    topBar: { links: TopBarLink[]; socialMedia: SocialMediaItem[] };
    items: NavItem[];
  };
}) {
  ...
  return (
    <>
      <div className="">
        {/* Top bar */}
        <div className="bg-white border-b border-gray-200 hidden md:block stick">
            ...
            <div className="flex items-center justify-between h-20">
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="flex items-center gap-2">
                  {navbarData.topBar.links.map((link, index) => (
            ...
            <div className="flex items-center justify-between h-16">
              ...
              <div className="md:hidden">
                <button
                  ...
                  className="inline-flex items-center justify-center p-2 text-gray-700 hover:text-main-100 hover:bg-gray-200 transition-colors"
            ...
            <div className="md:hidden border-t border-gray-200 bg-gray-100">
              <div className="px-2 pt-2 pb-3 space-y-1">
                    ...
                    {hasDropdown(item) ? (
                      <>
                        <button
                          ...
                          className="w-full flex items-center justify-between gap-2 px-3 py-2.5 text-base font-medium transition-colors hover:bg-main-100 hover:text-white text-gray-700"
                        >
                          <div className="flex items-center gap-2">
                            <span>{item.label}</span>
                          </div>
                          ...
                          <div className="mr-4 mt-1 space-y-1">
                            {(item.dropdown ?? []).map((dropdownItem) => (
                              <Link
                                ...
                                className={
                                  isActive(dropdownItem.href)
                                    ? "flex items-center gap-3 px-3 py-2.5 text-base transition-colors bg-main-100 text-white"
                                    : "flex items-center gap-3 px-3 py-2.5 text-base transition-colors hover:bg-main-100 hover:text-white text-gray-700"
                                }
                              >
                                <span>{dropdownItem.label}</span>
                              </Link>
                      ...
                      <Link
                        ...
                        className={
                          isActive(item.href)
                            ? "flex items-center gap-2 px-3 py-2.5 text-base font-medium transition-colors bg-main-100 text-white"
                            : "flex items-center gap-2 px-3 py-2.5 text-base font-medium transition-colors hover:bg-main-100 hover:text-white text-gray-700"
                        }
                      >
                        <span>{item.label}</span>
                      </Link>
                ...
                <div className="border-t border-gray-300 mt-4 pt-4">
                  {navbarData.topBar.links.map((link) => (
                    <Link
                      key={link.id}
                      href={link.href}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="flex items-center gap-2 px-3 py-2.5 text-base font-medium transition-colors hover:bg-main-100 hover:text-white text-gray-700"
                    >
